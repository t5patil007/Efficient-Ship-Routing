...
class BezierGCS(BaseGCS):
    ...
    def addHydrokineticCost(self, weight, region_vectors, integration_points=50):
        """
        Convex directional alignment cost using L2 norm penalties.
        
        Penalizes the perpendicular component of velocity relative to region vector.
        Weight can be scalar (uniform penalty) or vector (dimension-specific penalties).
        """
        if isinstance(weight, (float, int)):
            weight_matrix = weight * np.eye(self.dimension)
        elif isinstance(weight, (list, np.ndarray)):
            assert len(weight) == self.dimension, f"weight vector must have length {self.dimension}"
            weight_matrix = np.diag(weight)
        else:
            raise ValueError("weight must be scalar or vector of length dimension")
        
        if isinstance(region_vectors, list):
            region_vec_dict = {i: np.array(vec) for i, vec in enumerate(region_vectors)}
        else:
            region_vec_dict = region_vectors
        
        s_points = np.linspace(0., 1., integration_points + 1)
        u_path_deriv = self.u_r_trajectory.MakeDerivative(1)
        
        for region_idx, region_vec in region_vec_dict.items():
            region_norm_sq = np.dot(region_vec, region_vec)
            if region_norm_sq < 1e-6:
                continue
                
            region_unit = region_vec / np.sqrt(region_norm_sq)
            
            q_ds = u_path_deriv.vector_values(s_points)
            
            for ii in range(integration_points + 1):
                direction_vec = q_ds[:, ii]
                
                # Compute perpendicular direction vector
                perp_direction = direction_vec - np.dot(direction_vec, region_unit) * region_unit
                
                # Decompose the perpendicular direction expression
                H_perp = DecomposeLinearExpressions(perp_direction, self.u_vars)
                
                # Weight by trapezoidal rule and region magnitude
                if ii == 0 or ii == integration_points:
                    integral_weight = 0.5 * 1./integration_points
                else:
                    integral_weight = 1./integration_points
                
                # Apply weight matrix and scaling
                scaling = np.sqrt(integral_weight * region_norm_sq)
                A_cost = scaling * weight_matrix @ H_perp
                
                alignment_cost = L2NormCost(A_cost, np.zeros(A_cost.shape[0]))
                self.edge_costs.append(alignment_cost)
                
                for edge in self.gcs.Edges():
                    if edge.u() == self.source:
                        continue
                    target_idx = [v for v in self.gcs.Vertices()].index(edge.v())
                    if target_idx == region_idx:
                        edge.AddCost(Binding[Cost](alignment_cost, edge.xu()))
